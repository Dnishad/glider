...
THREE.CTMLoader.prototype.load = function (url, callback, parameters) {
	...
	xhr.onreadystatechange = function () {
		var gg = (xhr.getResponseHeader("Content-Length") / 1024000);
		if (xhr.readyState === 4) {
			...
			if (xhr.status === 200 || xhr.status === 0) {
				...
				if (parameters.useWorker) {
					...
					var worker = parameters.worker || new Worker("js/c/libs/CTMWorker.js");
...
function loaded(){
  if ( navigator.userAgent.indexOf("WebKit") == -1){
    var worker = new Worker("js/c/libs/CTMWorker.js");
    ...
    worker.postMessage(xhr.responseText);
      ...
      file =  new CTM.Stream(xhr.responseText) ;
   }
}
